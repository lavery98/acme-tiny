...
def get_crt(account_key, csr, acme_dir, log=LOGGER, CA=DEFAULT_CA):
	...
	for domain in domains:
		...
		proc = subprocess.Popen(["openssl", "dgst", "-sha256", "-binary"], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
		out, err = proc.communicate(keyauthorization.encode('utf8'))
		
		if proc.returncode != 0:
			raise IOError("OpenSSL Error: {0}".format(err))
		
		log.info("_acme-challenge." + domain + ". TXT " + _b64(out))
	...
	for domain in domains:
		...
		code, result = _send_signed_request(challenges[count][0]['uri'], {
			"resource": "challenge",
		...
		while True:
			try:
				resp = urlopen(challenges[count][0]['uri'])
	...
	log.info("All DNS records can now be removed")
